...
from python文件.selectDocWin import selectDocWin
from python文件.payingWin import payingWin
from python文件.chattingWin import chattingWin
from python文件.searchEssayWin import searchEssayWin
from python文件.seeMoreEssays import seeMoreEssays
from python文件.gradingWin import gradingWin
from python文件.illResume import illResume
...
class mainWin(QtWidgets.QWidget):
    ...
    def on_click_enterSelectDocWin(self):
        ...
        self.win_illResume.illDepiction = "家族病史：" +   self.win_illResume.answerEdit[0].text() +\
                            ...
                            "  FGO经历："  +   self.win_illResume.answerEdit[3].text() +\
                            "  睡眠，饮食状况：" + self.win_illResume.answerEdit[4].text() +\
                            "  哈皮举动："   + self.win_illResume.answerEdit[5].text() +\
...
#        db.commit()
    ...
    def on_click_payCompleted(self):#新建聊天窗口---------------------应该等聊天窗口关闭之后才能继续操作其他窗口
        ...
        sql = "INSERT INTO 病历 (患者ID, 存档时间, 主诉,医生ID) VALUES ('%s','%s','%s','%s')"%\
                (self.patientID,time.strftime("%Y-%m-%d"),self.win_illResume.illDepiction,self.win_selectDoc.selectedDoc)
        print(sql)
        db = mc.connect(host="localhost", user="root", password="caoqiming6666", database="hospital_management")
        cursor = db.cursor()
